...
public class ChatListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try {
            if (ModuleMoA4User.getInstance().getName().equals(chatMessageList.get(position).getName())) {
            ...
            if ("获取用户信息失败".equals(e.getMessage())) {
                Toast.makeText(holder.left_date.getContext(), "获取用户信息失败", Toast.LENGTH_SHORT).show();
            }
            e.printStackTrace();
        }
